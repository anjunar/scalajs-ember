...
/** What kind of DOM node something is, asked in a way that survives a second document.
  ...
  * compiles to `instanceof`, and `instanceof Text` checks against the '''current window's'''
  * `Text` constructor. A text node belonging to an iframe's document is an instance of that
  * frame's `Text`, not of this one -- so the test fails, the node is classified as "something
  * else", and a selection port reports a position it cannot map.
  *
  * That is exactly what happened: every mapping in an iframe came back as "not projected", with
  * the right elements sitting in the DOM the whole time. §15.4 asks for `ownerDocument` and
  * `defaultView` to be used for an editor host in an iframe, and this is the half of that rule
  * that is easy to miss -- it is not only about which `window` is asked, but about not assuming
  * there is only one.
  *
  * `nodeType` is a number defined by the DOM specification. It means the same thing in every
  * realm, which is the whole point.
  */
private[browser] object DomKinds:
  ...
  /** Whether an element can take focus, asked of the object rather than of its class.
    ...
    * HTMLElement` here. What matters is whether it has the behaviour, and a DOM object answers
    * that directly.
    */
