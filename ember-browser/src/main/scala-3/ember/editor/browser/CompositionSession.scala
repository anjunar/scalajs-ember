...
/** Which blocks a composition starts in.
  ...
  *
  * Because §15.3 forbids the narrow reading in so many words: "ein blockuebergreifender Start
  * darf nicht als Ein-Leaf-Fall behandelt werden". An IME replacement reaches further than the
  * run it started in -- across marks, across atom boundaries, across blocks -- and a barrier
  * drawn around one leaf would let the projection rewrite the rest of the sentence underneath a
  ...
  *
  * It is the one part of the composition protocol that is pure: a document and a selection go in,
  * a set of node ids comes out. Everything around it needs an engine.
  */
...
/** One run of native text input, from `compositionstart` to its completion.
  ...
  * erfassten Textes verhindert doppelte Einfuegung". The id is what lets a late event be
  * recognised as belonging to a composition that has already ended, which is the shape of most
  * of the browser bugs §15.2 lists.
  *
  * @param captured
  *   the last text read out of the DOM, and the node it came from. Compared on completion so that
  *   a final `input` after `compositionend` does not insert the same text twice.
  */
...
object CompositionSession:
  ...
  def start(document: DocumentRead, selection: Option[Selection], revision: Revision): CompositionSession =
    counter += 1
    CompositionSession(
      id = counter,
      baseRevision = revision,
...
/** A change that is not the running composition's, refused before it commits.
  ...
  *
  * Outside the protected area too, and that is the part worth explaining: the reason is not the
  * DOM but the history. §14 records a composition as '''one''' group, and a snapshot history
  * undoes a group whole. An independent commit landing inside that group would be taken back with
  * it -- someone else's edit, undone by a keystroke that had nothing to do with it. Allowing them
  * needs selective history, and §15.3 rules that out for the MVP by name.
  */
...
/** The pre-commit rule that enforces it.
  ...
  *
  * Because §10's step 5 is exactly the moment §15.3 names -- "vor Commit" -- and because a check
  * in the controller would only see what goes through the controller. A feature module's command,
  * a timer, an arriving upload: all of those reach the session directly, and all of them are
...
object CompositionGate:
  ...
  /** The tag a composition's own transactions carry.
    *
    * Named in the core (§14's typed metadata) because the rich-text profile reads the same word
    * to defer its merges (§8.2), and two modules that never meet cannot agree on a string each
    * spells for itself.
    */
  ...
  /** @param running
    *   the id of the composition in progress, if there is one. Read at check time, because a
    *   rule outlives any single composition.
    */
