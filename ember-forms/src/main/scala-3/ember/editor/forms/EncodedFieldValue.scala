...
/** The submit value of a field, and the revision it belongs to.
  ...
  * Revision." A string on its own cannot say whether it is current, and a form that writes a
  * value computed one commit ago sends a payload the user never saw.
  *
...
final case class EncodedFieldValue(revision: Option[Revision], value: String):
  ...
  /** Whether this value was computed for exactly this state.
    ...
    * mistaken for a current one -- and a form would send an empty payload for a document that
    * has content.
    */
...
/** The derived state field that carries a document's form value.
  ...
  *
  * §16: "Diese Formatgrenze wird '''vor Commit''' geprueft, auch fuer programmgesteuerte
  * Commands: Ein Form-Adapter installiert eine synchrone Darstellbarkeitsregel und ein
  * abgeleitetes `EncodedFieldValue`-StateField. Dessen Reducer berechnet gegen den fertig
  * normalisierten Kandidaten den Submit-String oder lehnt die Tx ab."
  *
  ...
  * with it.
  *
  * The consequence is the one §16 names: a `ToggleUnderline` in a strict CommonMark field does
  * not produce a document whose form value is stale. It produces no document at all -- the
  * transaction is rejected, and the session, the form value and the history are untouched.
  *
  ...
  * speichert keine veralteten Codec-Ergebnisse." So [[onHistoryRestore]] stays at `Recompute`,
  * which is the default -- and the default is right here for a reason worth stating: a captured
  * encode result is a cached answer to a question the document can answer again, and a cache in
  * a history snapshot is a cache nobody invalidates.
  *
  * @param fieldName
...
final class EditorField(val fieldName: String, val codec: FieldCodec)
  ...
  /** Recomputed for every candidate, not carried forward.
    ...
    * value has to exist for the '''first''' state too, before anything changed. [[reduce]] runs
    * on every commit and covers both.
    */
  ...
  override def reduce(
    ...
    codec.encode(candidate.document).map(value => EncodedFieldValue(Some(candidate.previous.revision.next), value))
  ...
  /** The value for a state, computed on demand.
    ...
    * [[initial]], which is the empty value rather than the document's. A form that read it
    * would send an empty payload for a non-empty document.
    */
...
/** Installs a field in a session.
  ...
  * abgeleitetes `EncodedFieldValue`-StateField." Installing is what an [[Extension]] does (§13),
  * so this is one -- and it carries the field alone, because in this design the rule and the
  * field are the same object.
  *
  * An application that wants two fields on one session registers two extensions with two names.
  * Two extensions with the same field would be a duplicate id, and the resolver refuses that.
  */
