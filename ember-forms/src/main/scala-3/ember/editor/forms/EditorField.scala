...
/** The two field formats §16 offers, and the reason there are exactly two.
  ...
  *
  *   - a '''Markdown''' field sends something a human wrote and can read, and refuses documents
  *     it cannot express;
  *   - a '''JSON''' field sends the document, losslessly, and is legible to a program rather
  *     than to a person.
  *
  * §16 is candid about the second one: "JSON-Felder koennen als universellen Non-JS-Fallback
  * eine beschriftete JSON-Textarea anbieten; anwendungsspezifische serverseitige Formulare sind
...
object EditorFields:
  ...
  /** A Markdown field.
    ...
    *   explizite verlustbehaftete Konvertierung mit Diagnose." `Strict` is the default, and
    *   that is the strict-CommonMark field §16 uses as its example: a `ToggleUnderline` in it
    *   cannot commit.
    */
  def markdown(
      name: String,
  ...
  /** A JSON field: the whole document, losslessly.
    *
    * No loss policy, because there is no loss to allow. A document that this codec cannot write
    * is a document with a node type nobody registered a codec for, and that is a configuration
    * error rather than a format boundary.
    */
...
private final class JsonFieldCodec(support: JsonSupport, pretty: Boolean) extends FieldCodec:
  ...
  def encode(document: Document): Either[EditorError, String] =
    val written =
      if pretty then DocumentJson.encode(document, support).map(value => JsonText.renderPretty(value))
      else DocumentJson.encodeToString(document, support)

    written.left.map(errors =>
      FieldError.NotRepresentable(name, errors.map(_.message))
    )
