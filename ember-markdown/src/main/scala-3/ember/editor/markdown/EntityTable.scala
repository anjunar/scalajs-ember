...
/** Which named character references a profile resolves.
  ...
  *
  * Because it is a '''subset''', and a subset has to be nameable. CommonMark defers to the
  * WHATWG list -- 2231 names, most of which no editor document has ever contained. Shipping it
  * would put roughly 150 kB of table into every browser bundle that links this module, for
  * `&angmsdaa;` and friends.
  *
  * So the default table is [[EntityTable.common]]: the five XML names, the whole Latin-1
  * supplement (which is what makes German, French and Scandinavian text survive a round trip),
  * and the punctuation and currency symbols that actually appear in prose. Roughly 150 entries.
  *
  * An application that needs more passes its own. That is the same shape as
  * `LinkUrlPolicy` and `MediaUrlPolicy`: the library picks a defensible default and the
  * application overrides it knowing that it did.
  *
  * ==Numeric references are complete==
  *
  * `&#35;` and `&#x1F600;` need no table, so there is no subset to apologise for. They are
  * handled in [[InlineParser]] and follow the specification exactly, U+0000 mapped to U+FFFD
  * included.
  */
...
object EntityTable:
  ...
  /** The default: what prose actually contains.
    *
    * Grouped by why each group is here, because "which entities" is exactly the kind of list
    * that grows by accident when nobody can say what belongs in it.
    */
  ...
  /** The Latin-1 letters, spelled out.
    *
    * This is the group that earns the table: without it `Gr&ouml;&szlig;e` survives a round
    * trip as `Gr&ouml;&szlig;e` and a German document quietly stops being German.
    */
  private def latin1Letters: Map[String, String] =
    ...
    val accents = Map(
      ...
      "circ"  -> Map('A' -> 'Â', 'E' -> 'Ê', 'I' -> 'Î', 'O' -> 'Ô', 'U' -> 'Û',
                     'a' -> 'â', 'e' -> 'ê', 'i' -> 'î', 'o' -> 'ô', 'u' -> 'û'),
      "tilde" -> Map('A' -> 'Ã', 'O' -> 'Õ', 'a' -> 'ã', 'o' -> 'õ'),
      "uml"   -> Map('A' -> 'Ä', 'E' -> 'Ë', 'I' -> 'Ï', 'O' -> 'Ö', 'U' -> 'Ü',
