...
final class Transaction private[core] (
  ...
  /** Setzt Dokument und Auswahl auf einen frueheren Stand.
    ...
    *
    * Es ist keine Operation. [[Operation]] beschreibt, was jemand '''tut'''; dies beschreibt, wohin
    * ein Stand zurueckgesetzt wird. Der Unterschied ist nicht akademisch: eine Operation kennt ihre
    * eigene Wirkung und liefert ChangeSet und Abbildung selbst, hier dagegen gibt es nur zwei
    * Staende, und der Unterschied muss ausgerechnet werden ([[DocumentDiff]]).
    *
    * ==Wofuer es da ist, und wofuer nicht==
    *
    * Fuer Undo und Redo (§14: "strukturell geteilte Document-Snapshots") und fuer alles, was einen
    * ganzen Stand ersetzt statt ihn zu bearbeiten -- ein verworfener Entwurf, ein neu geladenes
    * Dokument. '''Nicht''' als bequemer Ersatz fuer eine Bearbeitung: wer den naechsten Stand aus
    * dem aktuellen ausrechnet und ihn hier hineinreicht, verliert genau die Information, um die es
    * §10 geht, und laesst die Projektion einen Diff nachholen, den die Operation frei mitgeliefert
    * haette.
    *
    * ==Was geprueft wird==
    *
    * Dass das Dokument zum Schema der Sitzung gehoert. Mehr braucht es nicht: ein [[Document]] hat
    * die Invarianten aus §8.2 bereits erfuellt -- wer einen Wert dieses Typs in der Hand hat, haelt
    * ein gueltiges Dokument (§8.2). Die Auswahl wird wie bei [[setSelection]] gegen den
    * '''wiederhergestellten''' Stand geprueft, nicht gegen den, aus dem sie stammt.
    */
