...
/** Putting images into a document.
  ...
  * "Uploads sind ein Anwendungsservice. Browser-Datei, Progress und AbortSignal gehoeren zu
  * einem Browser-/Forms-Port, nicht zum Core-Node."
  *
  * What this module takes is a finished [[MediaReference]] -- something that already exists at
  * an address. §20: "Ein Upload liefert erst nach dauerhafter Speicherung eine validierte
  ...
  * It is one history step, it needs no lifecycle, and an undo removes the node without touching
  * a file anywhere (§20).
  */
object ImageEditing:
  ...
  /** Inserts an image where the caret is.
    *
    * At a text position the run is split, and the image goes between the halves -- it is an
    * inline atom, so it belongs '''between''' characters and not next to a paragraph.
    */
  def insert(
  ...
  /** Keeps the caller's id when it is free, and picks one when it is not.
    *
    * A caller that built the node itself usually has no id worth keeping; one that is restoring
    * a known image does. Refusing the second case would make the command useless for a paste.
    */
  private def freshId(
      scope: TransformScope,
      generator: NodeIdGenerator,
  ...
  /** Rewrites the image at the caret.
    *
    * `Replace` keeps identity (§10), so a bookmark on the picture survives a change of its alt
    * text -- which is the common edit and the one §20 asks to be editable.
    */
  ...
  /** The image the selection points at.
    *
    * An atom has no text position inside it, so a caret cannot stand '''in''' one. What a
    * selection can do is name it -- as a [[NodeSelection]], or as a child position whose
    * neighbour it is.
    */
...
/** Images as an extension.
  ...
  * invariant to repair: an image has no children to go wrong, and its fields are already typed
  * so that a wrong one cannot be built.
  *
  * @param policy
