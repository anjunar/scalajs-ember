...
/** Code blocks as HTML.
  ...
  * ==Why two tags==
  *
  * `<pre><code>` is what HTML has for this, and both halves earn their place: `pre` preserves
  * the whitespace -- without it every indentation and every blank line collapses -- and `code`
  * says what the content is, which is what a screen reader and a stylesheet read. §16 asks for
  * semantic HTML in the delivered version, and here that means both.
  *
  ...
  * `{highlight=3-5}` and its kin -- and belongs in the Markdown fence it came from (§18.2), not
  * in HTML that a reader gets. It stays in the document, so P18 can write it back.
  */
object CodeSupport:
