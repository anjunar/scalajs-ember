...
/** Images as HTML.
  ...
  *
  * `src` and `alt` always -- `alt` even when it is empty, because `alt=""` is what tells a screen
  * reader that a picture is decorative, and leaving the attribute out tells it to read the file
  * name instead (§20). `title`, `width` and `height` only when the document has them.
  *
  ...
  * before the picture arrives. They are already validated -- [[PositivePixels]] cannot hold a zero
  * or a negative -- so nothing here has to check them again.
  *
  * ==What does not happen==
  *
  * No load, no probe, no measurement. §20: "keine externe URL wird vom Parser oder SSR-Server
  * automatisch abgerufen", and P16's acceptance repeats it for rendering. This adapter turns a node
  * into attributes; whether the picture exists is the browser's question, asked later, and SSR
  * never asks it at all.
  */
object ImageSupport:
