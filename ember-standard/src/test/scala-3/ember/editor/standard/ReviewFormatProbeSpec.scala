...
final class ReviewFormatProbeSpec extends AnyFlatSpec with Matchers {
  ...
  private val schema = ExtensionResolver
    .resolve(
      Vector(
  ...
  private def html(source: String): Document = HtmlImport
    .imported(
      source,
      schema,
      StandardHtmlImport.everything(links, media),
  ...
  private def decode(source: String): Document = MarkdownCodec
    .decode(source, schema, markdown, NodeIdGenerator.sequential("m"), root)
    .toOption
    .get
    .document
  private def paragraph(parts: (String, MarkSet)*): Document = {
    val texts = parts.zipWithIndex.map { case ((text, marks), i) =>
      TextNode(NodeId(s"t$i"), text, marks)
    }.toVector
    Document
      .build(
        schema,
        ...
        Vector[EditorNode](
          ...
          ParagraphNode(NodeId("p"), texts.map(_.id))
        ) ++ texts
      )
      .toOption
      .get
  ...
  "HTML import" should "retain an outer strong mark through links" in {
    runs(html("<p><strong><a href='/x'>bold</a></strong></p>")).head.marks shouldBe MarkSet.of(
  ...
  it should "keep the horizontal rule through strict markdown export" in {
    ...
    withClue(source) {
      decode(source).inDocumentOrder.exists(_.isInstanceOf[ThematicBreakNode]) shouldBe true
    }
  }
  "Strict markdown export" should "retain marked text including trailing space" in {
    ...
    withClue(source) {
      val decoded = runs(decode(source))
      decoded.map(_.text).mkString shouldBe "hello "
      all(decoded.map(_.marks)) shouldBe MarkSet.of(StandardMarks.Strong)
    }
  }
  ...
  it should "retain all adjacent inline code runs" in {
    val source = encode(
      paragraph(
        "a" -> MarkSet.of(StandardMarks.InlineCode),
        "b" -> MarkSet.of(StandardMarks.InlineCode)
      )
    ).source
    withClue(source) { runs(decode(source)).map(_.text).mkString shouldBe "ab" }
  }
  it should "retain whitespace at either mark boundary and in all-space code" in {
    Vector(StandardMarks.Strong, StandardMarks.Emphasis, StandardMarks.InlineCode).foreach { mark =>
      Vector(" hello", "hello ", " hello ", " ", "  ").foreach { value =>
        val marks  = MarkSet.of(mark)
        val source = encode(paragraph(value -> marks)).source
        withClue(source) {
          val result = runs(decode(source))
          result.map(_.text).mkString shouldBe value
          all(result.map(_.marks)) shouldBe marks
        }
      }
    }
  }
