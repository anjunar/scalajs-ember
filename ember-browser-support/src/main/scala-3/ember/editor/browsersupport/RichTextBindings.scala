...
/** Which browser intent becomes which rich-text command.
  ...
  *
  * §7 forbids `ember-browser` from importing a feature, and the rule earns its keep here. An intent
  * is browser vocabulary -- `formatBold` is what the specification calls the event, not what this
  * editor calls the mark. A profile that has no bold has no binding for it, and the controller then
  * leaves the event native instead of swallowing it. That is not a special case; it is the same
  * mechanism for every intent nobody claims.
  *
  ...
  * sanitising. Until that module exists, paste stays native and `input` imports the result, which
  * is the honest state of affairs rather than a half one.
  */
object RichTextBindings:
  ...
  /** The formats this profile has. Superscript and subscript are not among them.
    *
    * §8.2 leaves the set to the profile, and an unbound format is not a gap -- it is an editor that
    * does not offer it. The browser is then free to do nothing, which is what it does.
    */
  ...
  /** The shortcuts §22 expects a rich-text editor to have.
    *
    * Plain Enter and Backspace are '''not''' here. §15.2: "Text generell ueber Input-Pipeline" -- a
    * keydown table cannot see dictation, autocorrect or a mobile keyboard. Shift+Enter is the one
    * exception, and the comment on it says why the engines forced it.
    */
