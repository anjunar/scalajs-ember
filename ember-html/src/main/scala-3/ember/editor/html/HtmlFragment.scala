...
/** Eine begrenzte, unveraenderliche HTML-Beschreibung fuer Import und Export.
  *
  * §19.1 ist hier ungewoehnlich deutlich: das ist "'''kein diffbarer View-Baum'''" und hat "keine
  * Mount-/Update-API". Der Unterschied ist keine Formsache. Ein Fragment mit Update-Laufzeit waere
  * ein zweiter Renderer neben UI -- genau das, was §2 als Non-Goal fuehrt und was §15.1 mit
  * "Generische DOM-Erzeugung, Besitz, Einfuegen, Verschieben und Entfernen gehoeren UI3"
  * ausschliesst.
  *
  * Ein Fragment entsteht also aus etwas und wird zu etwas. Es lebt nicht.
  *
  * Der Importparser folgt mit P24. Bis dahin dient dieser Typ der Ausgabeseite: die Serialisierung
  * nach HTML, gegen die sich SSR und Browser vergleichen lassen.
  */
enum HtmlFragment:
...
object HtmlFragment:
  ...
  /** Serialisiert nach HTML.
    ...
    * verglichen, und jedes eingefuegte Leerzeichen waere im Dokument ein Textknoten, den es dort
    * nicht gibt.
    */
  ...
  /** Nur was der Parser als Markup lesen wuerde. `>` bleibt stehen -- es ist im Textinhalt
    * eindeutig und ein zusaetzliches `&gt;` machte den Vergleich mit der Browserausgabe unnoetig
    * schwer.
    */
